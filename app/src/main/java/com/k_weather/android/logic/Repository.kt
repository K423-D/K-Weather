...
import com.k_weather.android.logic.model.Weather
    ...
    fun refreshWeather(lng: String, lat: String) = fire(Dispatchers.IO) {
        coroutineScope {
            val deferredRealtime = async {
                KWeatherNetwork.getRealtimeWeather(lng, lat)
            }
            val deferredDaily = async {
                KWeatherNetwork.getDailyWeather(lng, lat)
            }
            ...
            if (realtimeResponse.status == "ok" && dailyResponse.status == "ok") {
                ...
                Log.d("Refresh weather", weather.toString())
